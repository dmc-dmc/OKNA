...
let lastActiveWindowIndex = null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pridanie event listenera pre paste udalosť na celý dokument
...
function setupWindowEvents(index) {
    // Pridanie event listenera pre klik na kontajner okna, aby sme vedeli, ktoré okno je aktívne
    const windowContainer = document.querySelector(`.window-item[data-index="${index}"]`);
    if (windowContainer) {
    ...
    // Špecifický focus handler pre oblasť s obrázkom
    const imageContainer = document.getElementById(`window-image-${index}`);
    if (imageContainer) {
        imageContainer.addEventListener('click', () => {
            lastActiveWindowIndex = index;
            // Vizuálne zvýraznenie aktívnej oblasti
            document.querySelectorAll('.window-image-placeholder').forEach(el => {
                el.classList.remove('active-paste-target');
            });
            imageContainer.classList.add('active-paste-target');
        });
    }    
...
// Funkcia pre spracovanie paste eventu
function handlePaste(event) {
    // Kontrola, či existuje aktívne okno
    if (lastActiveWindowIndex === null) {
        return;
    }
    
    // Získanie obrázkov z clipboard
    ...
    // Ak sme našli obrázok, spracujeme ho
    if (imageItem) {
        const blob = imageItem.getAsFile();
        const reader = new FileReader();
        
        reader.onload = function(e) {
            // Uloženie obrázka do dát
            windowsData[lastActiveWindowIndex].image = e.target.result;
            
            // Aktualizácia UI - zobrazenie obrázka
            const imageContainer = document.getElementById(`window-image-${lastActiveWindowIndex}`);
            if (imageContainer) {
                imageContainer.innerHTML = '';
                const img = document.createElement('img');
                img.src = e.target.result;
                img.alt = `Okno ${lastActiveWindowIndex + 1}`;
                imageContainer.appendChild(img);
                ...
                deleteBtn.textContent = 'Odstrániť obrázok';
                deleteBtn.className = 'delete-image';
                deleteBtn.dataset.index = lastActiveWindowIndex;
                deleteBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    deleteWindowImage(lastActiveWindowIndex);
                });
                imageContainer.appendChild(deleteBtn);
            }
            ...
            showNotification('Obrázok bol úspešne vložený!');
        };
...
// Funkcia pre zobrazenie jednoduchej notifikácie
function showNotification(message) {
    // Vytvorenie notifikačného elementu
    const notification = document.createElement('div');
    notification.className = 'notification';
    notification.textContent = message;
    
    // Pridanie do dokumentu
